\label[chap:xsystem]
\chap The X Window system
The X Window System is a network transparent window system which runs on a wide range of computing and graphics machines.
It should be relatively straightforward to build the X.Org Foundation software distribution on most ANSI C and POSIX compliant systems.
Commercial implementations are also available for a wide range of platforms.~\cite[xman]

In this chapter, we will cover how X server works, with focus on the communication with its clients.
...
Each property has a type, which itself is a property.~\cite[xguide1]
...
In the figure~\ref[code:properties], you can see some of the properties retrieved from top-level window of an application.
...
We will talk discuss the meaning of some significant properties later on.

\codeblock{code:properties}{Window properties.}{properties.txt}
...
is ID of the window to which the event was sent, and other information specific to the event type.~\cite[xguide1]
