...
in a number of ways (e.g., scaled down, rotated), the X server does not have the information to decide which client
...
Walladge also mentions stability issues and comes to the conclusion that the time for switching to Wayland did not come yet.
...
Window is usually moved to the top of the stack by the window manager when user starts to interact with it.
...
They usually offer many different layouts of window placement and the user is able to dynamically switch between them.
...
Figure~\ref[fig:cwmmenu] shows cwm desktop with two windows and one of those menus.
