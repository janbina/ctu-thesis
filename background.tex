...
The main problem in X's approach that Wayland wants to solve, is that it does not isolate apps from each other.
...
The developer of its interface, David Smith, described it like this:
...
\caption/f The layout of a stacking window manager with overlapping windows
\endinsert
...
Windows cover the whole screen (with gaps), there are no overlaps.
\endinsert
...
This is described in one of its initial goals like this: ``Don’t be bloated, don’t be fancy
...
If it needs to be a bit bigger, it will be.''~\cite[i3]
...
Windows have a simple border and also a title bar, that shows only the name of the window.
It also comes with its panel called {\em i3bar}, which is used by default (see screenshot).
Since i3 supports EWMH, any other standalone panel could be used, though.
...
An example is depicted in figure~\ref[fig:i3desktop2].
There is a container split horizontally whose left child is a container that is using a tabbed layout with three windows,
and its right child is a container that is using stacking with three windows as well.
...
\caption/f Desktop of i3 with four open windows
\endinsert

\midinsert
\picw=12cm \cinspic img/i3wm_2.png
\clabel[fig:i3desktop2]{i3 desktop, tabs and stacks}
\caption/f Desktop of i3 with tabs and stacks
\endinsert
...
To mention a few more window managers, we can name Xfmw~\cite[xfce] or Kwin~\cite[kde], which are both stacking window managers
...
Another popular tiling window managers are herbstluftwm~\cite[herst] or Qtile~\cite[qtile].
