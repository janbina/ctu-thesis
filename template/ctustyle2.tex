...
\def\imgdir{template/img/}
\def\fontdir{template/font/}
...
\mtdef {listings}     {Listings}          {whatever}       {whatever}
...
% nums for tables and figures are defined in opmac
...
\else  % pdfTeX us used
   ...
   \ctustyleERR {Use luatex, i dont know how to configure those fonts in different directory}
...
\def\titlepage{
   ...
   \line{\locc \let\longlocalcolor=\localcolor
      ...
      \vbox to230mm{\advance\hsize by-8mm \parindent=0pt
         ...
         \line{\inspic{\imgdir ctulogo-\ifx\Blue\Grey bw-\fi new.pdf} \hss
...
\def\signature {\vskip1.5cm \indent\tocdotfill\null}
...
\def\abstractpage{
   ...
   \setbox\rightbox=\vbox{\sethsizefront
         \ifx\loadpattrs\undefined \def\initlanguage##1##2\message##3{##2}\fi
         \ehyph \the\abstractCZ \par
         \if&\the\keywordsCZ&\else{\bf\mtext{keywords}:} \the\keywordsCZ\par\fi
         \ifnum\orilang=\language \else
            \if&\the\titleCZ&\else{\bf\mtext{trans}:} \the\titleCZ
               \if&\the\subtitleCZ&\else \space(\the\subtitleCZ)\fi\fi\fi
         \vfil}
   ...
   \ifx\abstractpage\relax \else
      ...
      \ifvoid\leftbox
         \setbox\leftbox=\box0
         \insertoutline{Abstract/Abstrakt}
...
\def\tolpages{
   \setbox\tolbox=\vbox{\sethsizefront \parskip=0pt \iindent=9mm \tollist\vfil}
   \prepbox\tolbox
   \let\tmp=\do
   \ifvoid\tolbox \let\tmp=\relax \fi
   \ifx\tmp\do
      \insertoutline{\ifvoid\tolbox\else \mtext{listings}\fi}
      \setbox\leftbox=\vsplit\tolbox to\frontht
      \frontpage[\ifdim\wd\leftbox=0pt  \else\mtext{listings}\fi/%
                 \ifdim\wd\rightbox=0pt\fi]
      \loop \let\tmp=\relax
            \ifvoid\tolbox \else \setbox\leftbox=\vsplit\tolbox to\frontht
                                 \let\tmp=\do \fi
            \ifx\tmp\do \vfil\break \frontpage[/] \repeat
   \fi
}
...
\def\chaphook#1\relax{\numA=0 \numB=0 \numC=0 \numD=0 \numE=0 
   \secnum=0 \seccnum=0 \tnum=0 \fnum=0 \lnum=0 \dnum=0 \relax}
