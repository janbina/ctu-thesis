...
Applications usually deal with X events using so-called event loop, its typical implementation can be seen in Listing~\ref[code:xeventloop].
...
One example is setting border width of the window, which is not possible using the "xwindow.Window.Configure" method from Xgbutil, so
XGB's "xproto.ConfigureWindowChecked" must have been used.
...
According to the recommendation, "main" functions of both swm and swmctl are very minimal and they basically only invoke the code from the {\em internal} directory.
...
In Section~\ref[sec:xarch], it was already said that ``window manager is a client that has authority over the layout of windows on the
...
Xlib programming manual says~\cite[xguide1]: ``When the window manager selects SubstructureRedirectMask on the root window,
...
It is controlled via three swmctl commands -- example sxhkd mapping could be seen in Listing~\ref[code:cyclingshortcuts].
...
Martin Fl\"oser, former maintainer of KDE's window manager KWin, said in his 2012 article~\cite[unittesting] about window manager testing:
